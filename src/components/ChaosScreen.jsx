...
export default function ChaosScreen({ screamCount, onReset, onNavigate, userText = "" }) {
  ...
  const [screamData] = useState(() => {
    ...
    const entranceDelays = {
      ...
      centerWord: Math.random() * 0.1  // Faster - 0-0.1s instead of 0-0.3s
    };
  ...
  const [allEmojis] = useState(() => {
    ...
    return Array(densityLevel.emojiCount).fill(null).map(() => ({
      ...
      // Assign color from palette for mood ring effect
  ...
  return (
    ...
    >
      ...
      {/* Emojis with mood ring colors */}
      {allEmojis.map((emojiObj, index) => {
        const duration = 3 + Math.random() * 3;
        const delay = entranceDelays.emojis + (Math.random() * 0.5); // Stagger individual emojis slightly
        const startX = Math.random() * 100;
        const endX = Math.random() * 100;
        const startY = Math.random() * 100;
        const endY = Math.random() * 100;
        
        const xMovement = (endX - startX) * 2;
        const yMovement = (endY - startY) * 2;
      ...
      {/* Floating fortune (bounces around then settles below scream type) */}
      <AnimatePresence>
        {showFortune && (
          <motion.div
            className={`absolute ${fortuneFloated ? 'top-44 sm:top-52 left-1/2 -translate-x-1/2' : 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2'} z-45 px-8 py-4 rounded-2xl border-2`}
            style={fortuneFloated ? {
              background: `linear-gradient(135deg, ${palette.colors[0]}20, ${palette.colors[1]}20)`,
              borderColor: `${palette.colors[2]}60`,
          ...
          >
            <p className={`text-sm sm:text-base ${font} text-center max-w-xs`}
               style={{ 
                 color: fortuneFloated ? palette.colors[3] || palette.colors[0] : '#1f2937',
                 textShadow: fortuneFloated ? `1px 1px 2px ${palette.colors[0]}40` : 'none',
                 fontWeight: '600'
               }}
            >
              ✨ {fortune} ✨
      ...
      {/* Center word - ONE word that drifts around */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
        <motion.div
          ...
          transition={{ 
            duration: 8,
            delay: entranceDelays.centerWord,
            repeat: Infinity,
            ease: "easeInOut"
          }}
        >

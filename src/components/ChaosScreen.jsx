...
export default function ChaosScreen({ screamCount, onReset, onNavigate, userText = "" }) {
  ...
  return (
    ...
    >
      ...
      {/* Emojis with mood ring colors - SMOOTH motion */}
      {allEmojis.map((emojiObj, index) => {
        ...
        // Smooth continuous motion - no jumps
        const endX = startX + (Math.random() * 40 - 20); // Drift ±20% from start
      ...
      {/* Floating fortune (bounces around then settles below scream type) - SPECIAL */}
      <AnimatePresence>
        {showFortune && (
          <motion.div

...
export default function ChaosScreen({ screamCount, onReset, onNavigate, userText = "" }) {
  ...
  return (
    ...
    >
      ...
      {/* Floating fortune (magical appearance, smooth floating) - ON TOP OF EVERYTHING */}
      <AnimatePresence>
        {showFortune && (
          <motion.div
            className={`absolute ${fortuneFloated ? 'top-36 sm:top-40 left-1/2 -translate-x-1/2' : 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2'} px-8 py-4 rounded-2xl border-2`}
            ...
            animate={fortuneFloated ? { 
              ...
              opacity: 1,
              filter: 'blur(0px)',
              x: ['-50%', 'calc(-50% + 8px)', 'calc(-50% - 8px)', 'calc(-50% + 5px)', 'calc(-50% - 5px)', '-50%'],
              y: [0, -3, 3, -2, 2, 0]
            } : { 
              scale: [0, 1.1, 1],
              opacity: [0, 0.5, 1],
              filter: ['blur(10px)', 'blur(5px)', 'blur(0px)'],
              x: [0, 40, -40, 25, -25, 10, -10, 0],
              y: [0, -25, 25, -15, 15, -8, 8, 0]
            }}
      ...
      {/* Center word - positioned LOWER to avoid fortune, appears INSTANTLY */}

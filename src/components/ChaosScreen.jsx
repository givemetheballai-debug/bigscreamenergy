...
export default function ChaosScreen({ screamCount, onReset, onNavigate, userText = "" }) {
  ...
  const { screamType, fortune, palette, selectedShapes, densityLevel, font, shuffledWords, mainWord } = screamData;
  ...
  return (
    ...
    >
      ...
      {/* Floating fortune (appears then stays) */}
      <AnimatePresence>
        {showFortune && (
          <motion.div
            className={`absolute ${fortuneFloated ? 'top-32 sm:top-36' : 'top-1/3'} left-1/2 -translate-x-1/2 z-50 px-6 py-3 bg-white/90 backdrop-blur-md rounded-full`}
            initial={{ scale: 0, opacity: 0, y: 50 }}
            animate={{ 
              scale: 1, 
              opacity: 1, 
              y: fortuneFloated ? 0 : 0 
            }}
      ...
      {/* Center word - ONE word for whole scream */}

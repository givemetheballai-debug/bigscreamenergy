...
export default function ChaosScreen({ screamCount, onReset }) {
  ...
  const [scatteredPhrases] = useState(() => {
    return Array(15).fill(null).map(() => ({
      ...
      x: Math.random() * 90 + 5,
      y: Math.random() * 80 + 10,
      size: Math.random() > 0.5 ? 'text-xl sm:text-2xl md:text-4xl' : 'text-lg sm:text-xl md:text-3xl',
      delay: Math.random() * 2,
    ...
    <>
      ...
      {allEmojis.map((emoji, index) => {
        ...
        return (
          <motion.div
            key={index}
            className="absolute text-4xl sm:text-5xl md:text-7xl pointer-events-none"
            style={{ 
              left: `${startX}%`, 
              top: `${startY}%`,
      ...
      <motion.div
        key={currentWord}
        className="absolute top-1/2 left-1/2 z-30 w-full px-4"
        style={{
      ...
      >
        <h2 
          className="text-5xl sm:text-7xl md:text-9xl font-black text-white text-center whitespace-normal break-words"
          style={{ 
      ...
      >
        <div className="flex justify-center gap-3 sm:gap-6 text-xs sm:text-sm text-white/80 font-medium px-4 flex-wrap">
          ...
          <a href="/ranks" className="hover:text-white transition">Ranks</a>

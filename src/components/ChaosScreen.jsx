...
// Color palettes for gradients - LOTS of variations
const DARK_PALETTES = [
  ...
  ['#2c003e', '#1a0033'], // deep purple variations
  ['#0f0f0f', '#2d1b3d'], // near black to purple
  ['#1b1b3a', '#2d2d5c'], // dark blue purple
  ['#330033', '#1a001a'], // dark magenta
  ['#1a0d2e', '#3d2f5c'], // purple midnight
  ['#0a192f', '#1e3a5f'], // dark ocean blue
  ['#1c1c3a', '#3d2d5c'], // deep navy
];

const BRIGHT_PALETTES = [
  ['#ffd60a', '#b5f8fe'], // gold to light purple
  ['#06ffa5', '#ff5edf'], // neon green to hot pink
  ['#04c8de', '#ffd60a'], // cyan to gold
  ['#ff6b9d', '#c8b6ff'], // pink to lavender
  ['#ffa500', '#ff1493'], // orange to deep pink
  ['#00d4ff', '#7b2cbf'], // bright cyan to purple
  ['#39ff14', '#ff006e'], // neon green to magenta
  ['#ff69b4', '#ffb6c1'], // hot pink to light pink
];

// Function to create random shade variations - MORE DRAMATIC
const createShadeVariation = (palette) => {
  const vary = () => Math.floor(Math.random() * 80) - 40; // -40 to +40 (was -20 to +20)
  ...
  return [adjustColor(palette[0]), adjustColor(palette[1])];
};
...
export default function ChaosScreen({ screamCount, onReset, onNavigate }) {
  ...
  const [paletteData] = useState(() => {
    const basePalette = COLOR_PALETTES[Math.floor(Math.random() * COLOR_PALETTES.length)];
    // Apply random shade variation to make each scream unique
    const selectedPalette = createShadeVariation(basePalette);
    const isDark = DARK_PALETTES.some(p => {
      // Check if base palette matches any dark palette
      return basePalette[0] === p[0] || Math.abs(parseInt(basePalette[0].slice(1,3), 16) - parseInt(p[0].slice(1,3), 16)) < 30;
    });
    
    // Generate contrasting canvas colors based on darkness
    const canvasColors = isDark 
      ? [ // Bright, saturated colors for dark backgrounds
          `#${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}`,
          `#${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}`,
          `#${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}${Math.floor(Math.random() * 100 + 155).toString(16)}`
        ]
      : [ // Darker colors for bright backgrounds
          `#${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}`,
          `#${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}`,
          `#${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}${Math.floor(Math.random() * 100).toString(16).padStart(2,'0')}`
        ];
    
    return { palette: selectedPalette, isDark, canvasColors };
  });
  const palette = paletteData.palette;
  const isDarkPalette = paletteData.isDark;
  const canvasColors = paletteData.canvasColors;
  ...
  const [allEmojis] = useState(() => {
    ...
    // Create emojis with random sizes
    const emojiSizes = [
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      confetti({
        ...
        spread: 70,
        origin: { x: 0 },
        colors: [...palette, ...canvasColors, '#ffd60a', '#06ffa5', '#ff006e'],
        startVelocity: 45,
        gravity: 0.8,
      });
      confetti({
    ...
    return () => clearInterval(interval);
  }, [screamCount, palette, canvasColors]);

  // Generate falling confetti pieces with colors matching palette and canvas
  const [fallingConfetti] = useState(() => {
    const baseColors = ['#ff006e', '#06ffa5', '#ffd60a', '#04c8de', '#ff5edf', '#b5f8fe', '#fff'];
    // Add palette AND canvas colors to confetti for variety
    const colors = [...baseColors, palette[0], palette[1], ...canvasColors];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Layer 1 - Background blobs (slow) - BIGGER with contrasting colors
      for (let i = 0; i < 4; i++) {
        ...
        gradient.addColorStop(0, canvasColors[i % 3] + '50');
        gradient.addColorStop(1, canvasColors[(i + 1) % 3] + '30');
      ...
      // Dancing lines - BIGGER WAVES with contrasting colors
      for (let i = 0; i < 6; i++) {
        ctx.strokeStyle = canvasColors[i % 3] + 'CC';
        ctx.lineWidth = 5;
        ctx.beginPath();
      ...
      // Rotating triangles - BIGGER with contrasting colors
      for (let i = 0; i < 5; i++) {
        ...
        ctx.translate(x, y);
        ctx.rotate(rotation);
        ctx.strokeStyle = canvasColors[i % 3] + 'DD';
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(0, -size);
      ...
      // Diagonal slashes - MORE VISIBLE with contrasting colors
      for (let i = 0; i < 8; i++) {
        const opacity = Math.abs(Math.sin(time * 4 + i)) * 0.9;
        ctx.strokeStyle = canvasColors[i % 3] + Math.floor(opacity * 255).toString(16).padStart(2, '0');
        ctx.lineWidth = 8;
        ctx.beginPath();
      ...
      // Starbursts - BIGGER with contrasting colors
      for (let i = 0; i < 4; i++) {
        ...
        ctx.strokeStyle = canvasColors[i % 3] + 'AA';
        ctx.lineWidth = 3;
      ...
      // Circles expanding - NEW with contrasting colors
      for (let i = 0; i < 3; i++) {
    ...
    <>
      ...
      {allEmojis.map((emojiObj, index) => {
        ...
        return (
          <motion.div
            key={index}
            className={`absolute ${emojiObj.size} cursor-pointer`}
            style={{ 
              left: `${startX}%`, 
              top: `${startY}%`,

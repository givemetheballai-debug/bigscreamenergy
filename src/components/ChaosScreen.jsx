...
export default function ChaosScreen({ userText, onReset, onNavigate, globalCount }) {
  ...
  useEffect(() => {
    ...
    // Generate random line configurations (1-5 lines with random angles and positions)
    if (shapes.includes('lines')) {
      const numLines = randomInt(1, 5);
      const lines = Array.from({ length: numLines }, () => ({
        id: Math.random(),
        angle: randomInt(0, 180), // 0-180 degrees for varied diagonal/vertical/horizontal
        left: `${randomInt(10, 90)}%`, // random horizontal position
        top: `${randomInt(0, 100)}%`, // random vertical start
        length: randomInt(40, 100), // varied lengths
        thickness: randomInt(1, 3) // 1-3px thickness
      }));
      setLineConfigs(lines);
    }

    // Generate random positions for all other shapes
    setShapePositions({
      blob1: { top: `${randomInt(5, 30)}%`, left: `${randomInt(5, 40)}%`, size: randomInt(100, 180) },
      blob2: { bottom: `${randomInt(10, 30)}%`, right: `${randomInt(10, 40)}%`, size: randomInt(120, 200) },
      circle1: { top: `${randomInt(25, 50)}%`, left: `${randomInt(5, 30)}%`, size: randomInt(60, 100) },
      circle2: { bottom: `${randomInt(20, 40)}%`, right: `${randomInt(5, 30)}%`, size: randomInt(50, 80) },
      triangle: { top: `${randomInt(15, 40)}%`, right: `${randomInt(5, 30)}%`, size: randomInt(80, 120) },
      starburst: { top: `${randomInt(40, 60)}%`, right: `${randomInt(20, 50)}%`, size: randomInt(80, 120) },
      slash1: { top: `${randomInt(5, 25)}%`, right: `${randomInt(25, 50)}%`, length: randomInt(100, 160) },
      slash2: { bottom: `${randomInt(15, 35)}%`, left: `${randomInt(20, 45)}%`, length: randomInt(80, 140) }
    });
  ...
  return (
    ...
    >
      {/* Geometric shapes - animated during explosion, calm during fortune */}
      {selectedShapes.includes('blobs') && shapePositions.blob1 && (
        <>
          <motion.div 
            className="absolute rounded-full blur-2xl" 
            style={{ 
              backgroundColor: shapeColor,
              top: shapePositions.blob1.top,
              left: shapePositions.blob1.left,
              width: `${shapePositions.blob1.size}px`,
              height: `${shapePositions.blob1.size}px`
            }}
            initial={{ opacity: 0 }}
            animate={animationSpeed === 'fast' ? {
              opacity: [0, shapeOpacity, shapeOpacity],
              scale: shapeAnimation === 'pulse' ? [1, 2, 1] : 1,
              x: shapeAnimation === 'drift' ? [0, 80, 0] : 0,
          ...
          <motion.div 
            ...
            style={{ 
              backgroundColor: shapeColor,
              bottom: shapePositions.blob2.bottom,
              right: shapePositions.blob2.right,
              width: `${shapePositions.blob2.size}px`,
              height: `${shapePositions.blob2.size}px`
            }}
            initial={{ opacity: 0 }}
            animate={animationSpeed === 'fast' ? {
              opacity: [0, shapeOpacity, shapeOpacity],
              scale: shapeAnimation === 'pulse' ? [1, 2.2, 1] : 1,
              y: shapeAnimation === 'drift' ? [0, -80, 0] : 0,
      ...
      {/* Emoji stickers - varied motion styles, slow down during calm phase */}
      {emojis.map((item) => {
        ...
        return (
          <motion.div
            ...
            animate={{ 
              scale: [0, 1.2, 1],
              ...motionPattern,
              opacity: animationSpeed === 'fast' ? item.opacity : item.opacity * 0.6
            }}
            transition={{
      ...
      {/* Small scattered phrases - VISIBLE during explosion, fade during calm */}
      {smallPhrases.map((phrase) => (
        <motion.div
          ...
          animate={{ 
            scale: 1,
            rotate: [0, 5, -5, 0],
            opacity: animationSpeed === 'fast' ? 0.95 : 0.2
          }}
          transition={{
            delay: phrase.delay,
            scale: { duration: 0.3 },
      ...
      {/* Fortune - positioned between scream type and buttons, centered */}
      <AnimatePresence>
        {fortuneFloated && (
          <motion.div
          ...
          >
            <div 
              className="backdrop-blur-xl rounded-3xl p-8 sm:p-10 w-full max-w-2xl"
              style={{
            ...
            >
              ...
              <div className="space-y-5 text-center">
                ...
                {/* Advice/affirmation - bigger */}
                <motion.p 
                  className="text-xl sm:text-2xl md:text-3xl font-medium text-white leading-relaxed px-4"
                  style={{ 
                    fontFamily: 'Inter, system-ui, sans-serif',
                ...
                <p 
                  className="text-base sm:text-lg text-white/90 leading-relaxed px-4"
                  style={{ fontFamily: 'Inter, system-ui, sans-serif' }}
                >

...
// Extract emojis from user text (toned down boost)
...
export default function ChaosScreen({ userText, onReset, onNavigate, globalCount, screamCount }) {
  ...
  // Calculate rank when counter is expanded
  useEffect(() => {
    if (counterExpanded && screamCount) {
      setCurrentRank(getScreamRank(screamCount.total));
    }
  }, [counterExpanded, screamCount]);
  ...
  useEffect(() => {
    ...
    // Extract typed emojis for subtle boost
    const typedEmojis = userText ? extractEmojis(userText) : [];

    // Density - slightly higher on desktop
    const densityOptions = ['minimal', 'medium', 'packed'];
    let chosenDensity = randomChoice(densityOptions);
    // Desktop bump: minimal → medium, keep others same
    if (isDesktop && chosenDensity === 'minimal') chosenDensity = 'medium';
    setDensity(chosenDensity);

    // Pick 1-3 dominant shape types for this scream (slightly more on desktop)
    const numShapeTypes = isDesktop ? randomInt(2, 4) : randomInt(1, 3);
    const dominantShapes = shuffle(GEOMETRIC_SHAPES).slice(0, numShapeTypes);
    setSelectedShapes(dominantShapes);

    // Generate positions for each shape type (slightly more on desktop)
    const allShapeInstances = {};
    dominantShapes.forEach(shapeType => {
      const instanceCount = isDesktop ? randomInt(5, 15) : randomInt(3, 12);
      allShapeInstances[shapeType] = Array.from({ length: instanceCount }, () => ({
        id: Math.random(),
        top: `${randomInt(5, 70)}%`,
        left: `${randomInt(5, 95)}%`,
        size: isDesktop ? randomInt(50, 180) : randomInt(40, 150),
        rotation: randomInt(0, 360)
      }));
    });
    setShapePositions(allShapeInstances);

    // Confetti variations (more on desktop)
    const confettiParams = {
      count: isDesktop ? randomInt(50, 200) : randomInt(30, 150),
    ...
    let shuffledEmojis = shuffle(allEmojis);
    
    // TONED DOWN: Add typed emojis 2x each (not 3-5x)
    if (typedEmojis.length > 0) {
      const boostedEmojis = [];
      typedEmojis.forEach(emoji => {
        boostedEmojis.push(emoji, emoji); // Just 2x
      });
      shuffledEmojis = [...boostedEmojis, ...shuffledEmojis];
    }
    
    let emojiCount;
    if (chosenDensity === 'minimal') emojiCount = isDesktop ? randomInt(12, 18) : randomInt(8, 12);
    else if (chosenDensity === 'medium') emojiCount = isDesktop ? randomInt(28, 38) : randomInt(20, 30);
    else emojiCount = isDesktop ? randomInt(50, 65) : randomInt(45, 55);
    ...
    const emojiElements = shuffledEmojis.slice(0, emojiCount).map((emoji, i) => {
      ...
      else baseDuration = randomInt(3, 5); // intense - bumped from 2-4 to 3-5 for smoother animation

      // SUBTLE: Typed emojis slightly bigger/more visible
      const isTypedEmoji = typedEmojis.includes(emoji);
      const possibleSizes = isTypedEmoji 
        ? ['text-3xl', 'text-4xl', 'text-5xl'] // Modest boost
      ...
      return {
        ...
        x,
        y,
        size: randomChoice(possibleSizes),
        delay: Math.random() * 0.5,
        duration: baseDuration,
        rotation: randomInt(-45, 45),
        opacity: isTypedEmoji ? 0.9 : Math.random() * 0.4 + 0.6 // Slightly brighter if typed
      };
    });
    setEmojis(emojiElements);

    // Small phrases (more on desktop)
    let phraseCount;
    if (chosenDensity === 'minimal') phraseCount = isDesktop ? randomInt(8, 12) : randomInt(5, 8);
    else if (chosenDensity === 'medium') phraseCount = isDesktop ? randomInt(15, 20) : randomInt(10, 15);
    else phraseCount = isDesktop ? randomInt(25, 32) : randomInt(18, 25);
    ...
    // Generate sparkles (special effects)
    const sparkleCount = randomInt(15, 30);
    const sparklesList = Array.from({ length: sparkleCount }, (_, i) => ({
      id: i,
      left: `${randomInt(10, 90)}%`,
      top: `${randomInt(10, 90)}%`,
      delay: randomInt(0, 20) / 10,
      duration: randomInt(10, 20) / 10
    }));
    setSparkles(sparklesList);
    ...
    // 6s mark: Magic message starts exit (0.6s animation)
    const magicExitTimer = setTimeout(() => {
      setShowCalmingMessage(false);
    }, 6000);

    // 6.5s mark: Fortune appears (while magic message is fading out)
    ...
    return () => {
      ...
      clearTimeout(magicExitTimer);
      clearTimeout(fortuneTimer);
    };
  }, [userText, isDesktop, globalCount]);
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (navigator.share && !isDesktop) {
        ...
        const file = new File([blob], `big-scream-energy-${Date.now()}.png`, { type: 'image/png' });
        
        await navigator.share({
          files: [file],
          title: 'My Big Scream Energy',
          text: `I just let out my ${screamType?.name}!`
        });
      } else {
  ...
  const handleShareLink = () => {
    const shareText = `Sometimes you just need to scream 😤 I just let out my ${screamType?.name}. Try it: bigscreamenergy.com`;
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute top-20 sm:top-8 md:top-12 left-0 right-0 z-40 px-4"
      ...
      >
        <div className="text-center">
          ...
          <h1 
            className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl ${font} font-black text-white mb-1 sm:mb-2`}
            style={{ 
              textShadow: `0 0 10px ${palette.colors[0]}40, 0 2px 4px rgba(0,0,0,0.6)`,
              letterSpacing: '2px'
            }}
          >
            {screamType.name}
          ...
          <motion.div
            ...
            transition={{
              ...
              rotate: { 
                ...
                repeat: Infinity,
                ease: "easeInOut" // Changed from linear for smoother spinning
              },
              y: { 
                delay: item.delay,
                duration: currentDuration,
        ...
        {fortuneFloated && (
          <>
            ...
            {[...Array(12)].map((_, i) => {
              ...
              return (
                <motion.div
                  key={`fortune-sparkle-${i}`}
                  className="absolute text-yellow-200 text-xl sm:text-2xl z-65 pointer-events-none"
                  style={{
                    left: '50%',
                    top: '25%',
                    marginLeft: `${x}px`,
          ...
          >
            <div 
              className="backdrop-blur-xl rounded-3xl p-4 sm:p-6 md:p-8 w-full max-w-xl"
              style={{
            ...
            >
              ...
              <div className="space-y-2 sm:space-y-3 text-center">
                {/* Affirmation - smaller, supporting text */}
                <p 
                  className="text-sm sm:text-base md:text-lg font-medium text-white leading-relaxed"
                  style={{ 
                    fontFamily: 'Inter, system-ui, sans-serif',
                    textShadow: `0 0 10px ${palette.colors[0]}40, 0 2px 4px rgba(0,0,0,0.8)`
                  }}
                >
                  {fortune}
                </p>
                ...
                {/* YOUR FORTUNE header - bigger */}
                <p 
                  className="text-xl sm:text-2xl md:text-3xl font-semibold tracking-[0.3em] uppercase mb-2"
                  style={{ 
                    color: palette.colors[1],
                    fontFamily: 'Inter, system-ui, sans-serif',
                    textShadow: `0 0 20px ${palette.colors[1]}50`
                  }}
                >
                  Your Fortune
                </p>
                ...
                {/* Cosmic prediction - MAIN FEATURED ELEMENT */}
                <motion.p 
                  className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-white leading-relaxed px-2 pt-2"
                  style={{ 
                    fontFamily: 'Inter, system-ui, sans-serif',
                    textShadow: `0 0 25px ${palette.colors[1]}60, 0 0 50px ${palette.colors[0]}40, 0 4px 8px rgba(0,0,0,0.8)`
                  }}
                  animate={{
                    textShadow: [
                      `0 0 25px ${palette.colors[1]}60, 0 0 50px ${palette.colors[0]}40, 0 4px 8px rgba(0,0,0,0.8)`,
                      `0 0 40px ${palette.colors[1]}80, 0 0 80px ${palette.colors[2] || palette.colors[0]}60, 0 4px 8px rgba(0,0,0,0.8)`,
                      `0 0 25px ${palette.colors[1]}60, 0 0 50px ${palette.colors[0]}40, 0 4px 8px rgba(0,0,0,0.8)`
                    ]
                  }}
                  transition={{
      ...
      {/* Scream Counter - Expandable - Desktop: right side, Mobile: between fortune and buttons */}
      <AnimatePresence>
        {fortuneFloated && screamCount && (
          <motion.div
            className={isDesktop 
              ? "absolute top-4 right-4 max-w-[300px]"
              : "absolute bottom-36 left-0 right-0 flex justify-center px-4"
            }
          ...
          >
            <motion.div
              className="backdrop-blur-md rounded-2xl text-white/90 text-xs sm:text-sm font-medium overflow-visible pointer-events-auto relative"
              style={{
                background: 'rgba(0, 0, 0, 0.4)',
                border: `1px solid ${palette.colors[0]}40`,
                minWidth: counterExpanded ? '280px' : 'auto',
                zIndex: 100
              }}
              animate={{ 
                width: counterExpanded ? 'auto' : 'auto',
                height: counterExpanded ? 'auto' : 'auto'
              }}
              transition={{ duration: 0.3 }}
            >
              {!counterExpanded ? (
                // Compact state - clickable
                <motion.button
                  onClick={() => setCounterExpanded(true)}
                  className="px-4 py-2 w-full text-center pointer-events-auto relative"
                  style={{ zIndex: 101 }}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  {screamCount.daily} {screamCount.daily === 1 ? 'scream' : 'screams'} today / {screamCount.total} total
                </motion.button>
              ) : (
                // Expanded state - shows rank and button
                <motion.div
                  className="px-4 py-3 flex flex-col gap-2 min-w-[280px] pointer-events-auto relative"
                  style={{ zIndex: 101 }}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                >
                  <div className="text-center">
                    <div className="text-xs opacity-80 mb-1">Your Stats</div>
                    <div className="text-sm font-semibold">
                      {screamCount.daily} today / {screamCount.total} total
                    </div>
                    <div className="text-sm mt-1">
                      Rank: <span className="font-bold" style={{ color: palette.colors[1] }}>{currentRank}</span>
                    </div>
                  </div>
                  <div className="flex gap-2 justify-center">
                    <button
                      ...
                      className="px-3 py-1 text-xs rounded-full bg-white/10 hover:bg-white/20 transition pointer-events-auto relative cursor-pointer"
                      style={{ zIndex: 102 }}
                    >
                      Close
                    </button>
                    <button
                      ...
                      className="px-3 py-1 text-xs rounded-full font-semibold transition pointer-events-auto relative cursor-pointer"
                      style={{
                  ...
                  <div className="text-xs opacity-60 text-center">
                    (Leaving will lose your fortune)
                  </div>
                </motion.div>
              )}
            </motion.div>

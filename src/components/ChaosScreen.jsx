...
export default function ChaosScreen({ screamCount, onReset, onNavigate, userText = "" }) {
  ...
  return (
    ...
    >
      ...
      {/* Floating fortune (bounces around then settles lower) */}
      <AnimatePresence>
        {showFortune && (
          <motion.div
            className={`absolute ${fortuneFloated ? 'bottom-40 sm:bottom-44 left-1/2 -translate-x-1/2' : 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2'} z-50 px-6 py-3 bg-white/90 backdrop-blur-md rounded-full`}
          ...
          >
            <p className={`text-sm sm:text-base ${font} text-gray-800 text-center max-w-xs whitespace-nowrap`}>
              ✨ {fortune} ✨
            </p>
          </motion.div>
        )}
      ...
      {/* Scream type (stays on screen for screenshot) */}
      {fortuneFloated && (
        <motion.div
          className="absolute top-20 sm:top-24 left-0 right-0 z-40 px-4"

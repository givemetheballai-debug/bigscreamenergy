...
export default function ChaosScreen({ userText, onReset, onNavigate, globalCount }) {
  ...
  useEffect(() => {
    ...
    // Extract typed emojis
    const typedEmojis = extractEmojis(userText);

    // Density - MORE on desktop
    ...
    if (isDesktop && chosenDensity === 'medium') chosenDensity = 'packed';
    setDensity(chosenDensity);

    // Pick MORE shape types on desktop
    const numShapeTypes = isDesktop ? randomInt(3, 5) : randomInt(1, 3);
    const dominantShapes = shuffle(GEOMETRIC_SHAPES).slice(0, numShapeTypes);
    setSelectedShapes(dominantShapes);

    // Generate shooting stars
    const starCount = randomInt(5, 10);
    const stars = Array.from({ length: starCount }, (_, i) => ({
      id: i,
      left: `${randomInt(0, 100)}%`,
      delay: randomInt(0, 30) / 10,
      duration: randomInt(15, 25) / 10,
      size: randomInt(2, 4)
    }));
    setShootingStars(stars);

    // Generate sparkles
    const sparkleCount = randomInt(20, 40);
    ...
    // Generate positions for each shape type (MORE instances on desktop)
    const allShapeInstances = {};
    dominantShapes.forEach(shapeType => {
      const instanceCount = isDesktop ? randomInt(8, 18) : randomInt(3, 12);
      allShapeInstances[shapeType] = Array.from({ length: instanceCount }, () => ({
        id: Math.random(),
        top: `${randomInt(5, 70)}%`,
        left: `${randomInt(5, 95)}%`,
        size: isDesktop ? randomInt(60, 200) : randomInt(40, 150),
    ...
    const confettiParams = {
      count: isDesktop ? randomInt(60, 250) : randomInt(30, 150),
    ...
    // BOOST typed emojis - add them multiple times at the start
    if (typedEmojis.length > 0) {
      const boostedEmojis = [];
      typedEmojis.forEach(emoji => {
        // Add each typed emoji 3-5 times
        const boostCount = randomInt(3, 5);
        for (let i = 0; i < boostCount; i++) {
          boostedEmojis.push(emoji);
        }
      });
    ...
    let emojiCount;
    if (chosenDensity === 'minimal') emojiCount = isDesktop ? randomInt(15, 25) : randomInt(8, 12);
    else if (chosenDensity === 'medium') emojiCount = isDesktop ? randomInt(35, 50) : randomInt(20, 30);
    else emojiCount = isDesktop ? randomInt(70, 90) : randomInt(45, 55);
    ...
    const emojiElements = shuffledEmojis.slice(0, emojiCount).map((emoji, i) => {
      ...
      return {
        ...
        opacity: isTypedEmoji ? Math.random() * 0.3 + 0.7 : Math.random() * 0.4 + 0.6,
        isTyped: isTypedEmoji
      };
    });
    ...
    let phraseCount;
    if (chosenDensity === 'minimal') phraseCount = isDesktop ? randomInt(10, 15) : randomInt(5, 8);
    else if (chosenDensity === 'medium') phraseCount = isDesktop ? randomInt(18, 25) : randomInt(10, 15);
    else phraseCount = isDesktop ? randomInt(30, 40) : randomInt(18, 25);
  ...
  const handleShareLink = () => {
    const shareText = `I just released some Big Scream Energy! ${screamType?.name} 🔥\n\nTry it yourself: https://bigscreamenergy.com`;
  ...
  const handleEmojiClick = (emoji, x, y) => {
    ...
    setExplosionEmojis(prev => [...prev, newExplosion]);
    
    setTimeout(() => {
      setExplosionEmojis(prev => prev.filter(e => e.id !== newExplosion.id));
    }, 1000);
  };
  ...
  const gradientStyle = () => {
    ...
    if (gradientType === 'radial') return `radial-gradient(circle at center, ${color1}, ${color2})`;
    if (gradientType === 'vertical') return `linear-gradient(to bottom, ${color1}, ${color2})`;
    if (gradientType === 'horizontal') return `linear-gradient(to right, ${color1}, ${color2})`;
    return `linear-gradient(135deg, ${color1}, ${color2})`;
  };

  const renderShape = (shapeType, instances) => {
    return instances.map((instance) => {
      const colorIndex = Math.floor(Math.random() * palette.colors.length);
      const shapeColor = `${palette.colors[colorIndex]}${Math.floor(shapeOpacity * 255).toString(16).padStart(2, '0')}`;

      const getShapePath = () => {
        switch(shapeType) {
          case 'blobs':
            return 'M 50 10 Q 80 20, 90 50 Q 80 80, 50 90 Q 20 80, 10 50 Q 20 20, 50 10';
          case 'circles':
            return 'M 50 90 A 40 40 0 1 1 50 10 A 40 40 0 1 1 50 90';
          case 'triangles':
            return 'M 50 10 L 90 90 L 10 90 Z';
          case 'squares':
            return 'M 20 20 L 80 20 L 80 80 L 20 80 Z';
          case 'hexagons':
            return 'M 50 10 L 80 30 L 80 70 L 50 90 L 20 70 L 20 30 Z';
          case 'stars':
            return 'M 50 10 L 61 40 L 92 40 L 68 58 L 79 90 L 50 70 L 21 90 L 32 58 L 8 40 L 39 40 Z';
          case 'spirals':
            return 'M 50 50 Q 70 30, 80 50 Q 70 70, 50 70 Q 30 70, 20 50 Q 30 30, 50 30';
          case 'crescents':
            return 'M 30 10 Q 50 30, 30 50 Q 60 50, 80 30 Q 60 10, 30 10';
          case 'diamonds':
          ...
          case 'squiggles':
            return 'M 10 50 Q 30 20, 50 50 Q 70 80, 90 50';
          case 'starbursts':
            return 'M 50 10 L 55 45 L 90 50 L 55 55 L 50 90 L 45 55 L 10 50 L 45 45 Z';
          case 'slashes':
            return 'M 20 10 L 30 10 L 80 90 L 70 90 Z';
          default:
            return 'M 50 10 L 90 90 L 10 90 Z';
        }
      };
      ...
      return (
        ...
        >
          <path d={getShapePath()} fill={shapeColor} />
        </motion.svg>
      );
    });
  };
  ...
  return (
    ...
    >
      ...
      {/* Geometric shapes */}
      {selectedShapes.map((shapeType) => 
        shapePositions[shapeType] && renderShape(shapeType, shapePositions[shapeType])
      )}

      {/* Big scream word - VISIBLE during explosion, fades during calm */}
      <AnimatePresence>
        {showBigWord && (
          <motion.div
          ...
          >
            <h2 
              className={`text-6xl sm:text-8xl md:text-9xl ${font} font-black text-white`}
              style={{ 
                textShadow: `4px 4px 0 ${palette.colors[0]}, -4px -4px 0 ${palette.colors[2] || palette.colors[1]}`,
                WebkitTextStroke: `2px ${palette.colors[1]}`
              }}
            >
              {mainWord}
            </h2>
      ...
      {emojis.map((item) => {
        ...
        return (
          <motion.div
            ...
            animate={{ 
              scale: [0, item.isTyped ? 1.4 : 1.2, item.isTyped ? 1.1 : 1],
              ...motionPattern,
      ...
      {smallPhrases.map((phrase) => {
        ...
        return (
          <motion.div
            ...
            transition={{
              scale: { delay: phrase.delay, duration: 0.3 },
              opacity: { delay: phrase.delay, duration: 0.2 },
              rotate: { delay: phrase.delay, duration: 0.4, ease: "easeOut" }
            }}
          >
            {phrase.text}
          </motion.div>
      ...
      {/* Fortune - appears after calming message, positioned at 25% from top */}
      <AnimatePresence>
        {fortuneFloated && (
          <motion.div
            className="absolute left-0 right-0 z-70 px-4 sm:px-8 pointer-events-none"
            style={{ top: '25%' }}
          ...
          >
            <div className="max-w-2xl mx-auto text-center">
              ...
              >
                <p 
                  className={`text-2xl sm:text-3xl md:text-4xl ${font} font-bold text-white mb-4 leading-tight`}
                  style={{
                    textShadow: `
                      0 0 20px ${palette.colors[0]},
                      0 0 40px ${palette.colors[1]},
                      0 4px 8px rgba(0,0,0,0.8)
                    `
                  }}
                >
                ...
                <p 
                  className={`text-base sm:text-lg md:text-xl ${font} text-white/90 mb-2`}
                  style={{
                ...
                <p 
                  className={`text-sm sm:text-base ${font} text-white/70 italic`}
                  style={{
                    textShadow: `0 2px 4px rgba(0,0,0,0.6)`
                  }}
                >
                  You channeled a <span className="font-bold text-white">{screamType.name}</span> {connectorPhrase}
                </p>
              </motion.div>
            </div>
          </motion.div>
      ...
      {/* Counter - Mobile: above buttons, Desktop: top right */}
      <AnimatePresence>
        {fortuneFloated && (
          <motion.div 
            className={isDesktop 
              ? "absolute top-4 right-4 z-50"
              : "absolute bottom-48 left-0 right-0 flex justify-center z-50"
            }
          ...
          >
            <motion.button
              ...
              className="px-4 py-2 backdrop-blur-md text-white font-medium text-sm rounded-full shadow-lg"
              style={{
                background: `linear-gradient(135deg, ${palette.colors[0]}99, ${palette.colors[1]}99)`,

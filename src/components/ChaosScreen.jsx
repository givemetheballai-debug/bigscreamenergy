...
export default function ChaosScreen({ screamCount, onReset }) {
  ...
  const [scatteredPhrases] = useState(() => {
    return Array(15).fill(null).map(() => ({
      ...
      x: Math.random() * 90 + 5,
      y: Math.random() * 80 + 10,
      size: Math.random() > 0.5 ? 'text-2xl md:text-4xl' : 'text-xl md:text-3xl',
      delay: Math.random() * 2,
  ...
  return (
    <>
      ...
      <motion.div 
        className="absolute bottom-10 left-0 right-0 flex flex-wrap justify-center gap-3 px-4 z-50"
